...
const App: React.FC = () => {
      ...
      if (userHasWallet(userContext)) {
        ...
        getUserByWalletAddress(userContext.solana.address).then(userDb => {
          ...
          if (userDb) {
            ...
            connection.getBalance(new PublicKey(userContext.solana.address)).then(balance => setSolBalance(balance))
  ...
  const handleRedo = () => {
    if (redoStack.length > 0) {
      ...
      console.log("After Redo - History:", [...stateHistory, nextState].length, "Redo:", newRedoStack.length)
    }
  }
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'} font-pixel`}>
      ...
      <div className="container mx-auto px-4 py-8">
        <header className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-4">
            <button
              className={`p-2 ${isDarkMode ? 'bg-gray-800' : 'bg-white'} border-2 border-orange-500 rounded-lg shadow-pixel`}
            ...
            {!user && (
              <button
                className={`flex items-center gap-2 py-2 px-4 bg-purple-600 hover:bg-purple-700 text-white rounded-lg shadow-pixel transition-all ${isLoading ? 'opacity-70 cursor-not-allowed' : ''}`}
              ...
              <div className="relative" ref={userMenuRef}>
                ...
                {showUserMenu && (
                  <div className="absolute right-6 mt-2 w-64">
                    ...
                    {/* Menu content */}
                    <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} border-4 ${isDarkMode ? 'border-gray-700' : 'border-gray-300'} rounded-xl shadow-pixel z-50 p-4 relative`}>
                      <div className="space-y-4">
                        ...
                        <div className="border-b-2 border-orange-500 pb-2">
                          <div className="flex items-center">
                            <Wallet size={16} className="mr-2 text-orange-500" />
                        ...
                        <div className="text-xs opacity-80 border-b-2 border-orange-500 pb-2">
                          <div className="flex items-start">
                            <Info size={14} className="mt-1 mr-1 text-orange-500" />
                            <p>Send SOL to your wallet address to make deposits. Wallets are non-custodial.</p>
                          </div>
                        </div>
        ...
        <main className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1">
            {/* Canvas Container */}
            <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} p-6 rounded-xl border-4 ${isDarkMode ? 'border-gray-700' : 'border-gray-300'} shadow-pixel`}>
              <div className="flex justify-between mb-4">
                <h2 className="text-xl font-bold text-orange-500">Pixel Canvas</h2>
